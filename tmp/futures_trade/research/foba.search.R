...
tol.level <- as.numeric(args[[6]])
signal.thres.begin <- as.numeric(args[[7]])
signal.thres.end <- as.numeric(args[[8]])
signal.thres.step <- as.numeric(args[[9]])
vol.thres.begin <- as.numeric(args[[10]])
vol.thres.end <- as.numeric(args[[11]])
vol.thres.step <- as.numeric(args[[12]])
write.file <- as.numeric(args[[13]])
output.file.name <- args[[14]]
...
for (i in 1:num.fold) {
  ...
  cat("Local R2:\n")
  print(r2.ts.sparse)
  cat(paste("Max adjusted R2:", max(r2.ts.sparse), "\n\n"))
  coef.idx <- c()
  ...
  r2.global[i] <- summary(model.linear.local)$adj.r.squared
}
...
cat("\n")
cat(paste("Average out-of-sample adjusted R2:", mean(r2.global), "and sd of R2:", sd(r2.global), "\n\n"))
