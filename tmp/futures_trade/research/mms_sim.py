...
def runFile(contractSpec_file, configSpec_file, filename, log_folder, stop_loss, take_profit, bandwidth, threshold, volthreshold, ordersize, exchange, latency_ms, binary_name):
    date = os.path.basename(filename[0]).split(".")[0]
    date = date[:4]+'-'+date[4:6]+'-'+date[6:]
    contract = os.path.basename(filename[0]).split(".")[1]
    logfile = os.path.join(log_folder, "mms_%s_%s.log" % (date, contract))
    contract_spread = os.path.basename(filename[1]).split(".")[1]
    cmd = "$ZZTRADERBIN/%s %s %s %s %s %s %s %s %f %f %f %d %d %f %f %s %s"%(
        binary_name,
        contractSpec_file, configSpec_file, filename[0], date, exchange, contract, log_folder, stop_loss, take_profit, threshold, ordersize, latency_ms, volthreshold, bandwidth, filename[1], contract_spread)
    print cmd
    os.system(cmd)
...
def runAll(data_folder, log_folder, contractSpec_file, configSpec_file, exchangeCode, contractCode, stop_loss, take_profit, bandwidth, threshold, volthreshold, ordersize, latency_ms, binary_name, alias):
    filenames = identifyPairs(data_folder, contractCode)
    filenames.sort()
    result = defaultdict(list)
    for filename in filenames:
        print "processing", map(lambda f : os.path.basename(f), filename)
        tmp = runFile(contractSpec_file, configSpec_file, filename, log_folder, stop_loss, take_profit, bandwidth, threshold, volthreshold, ordersize, exchangeCode, latency_ms, binary_name)
        for key in tmp.keys():
...
def writeresult(result, output_folder, output_file):
    file1 = os.path.join(output_folder, "s%.2ft%.2fw%.2fh%.2fv%.2f.csv"%(result['stoploss'], 
                                                                         result['takeprofit'],
                                                                         result['bandwidth'],
                                                                         result['threshold'],
                                                                         result['volthreshold']))
    ...
    if os.path.exists(file2):
        f2 = open(file2, "a")
        row = [str(summaryline[key]) for key in ['contract', 'stoploss', 'takeprofit', 'bandwidth', 'threshold', 'volthreshold', 
                                                 'nday', 'totalpnl', 
        ...
        f2 = open(file2, 'w')
        row = [str(key) for key in ['contract', 'stoploss', 'takeprofit', 'bandwidth', 'threshold', 'volthreshold', 
                                    'nday', 'totalpnl', 
                                    'totalvolume', 'meanpnl', 'ppv', 'sharpe']]
        f2.write(",".join(row)+"\n")
        row = [str(summaryline[key]) for key in ['contract', 'stoploss', 'takeprofit', 'bandwidth', 'threshold', 'volthreshold', 
...
if __name__ == "__main__":
    ...
    parser.add_option("-w", "--bandwidth", dest="bandwidth", default=300,
                      help="bandwidth")
    parser.add_option("-e", "--threshold", dest="threshold", default=0.2,
                      help="threshold")
    ...
    result = runAll(data_folder, output_folder, contractSpec_file, configSpec_file, exchange, contract, options.stoploss, options.takeprofit, options.bandwidth, options.threshold, options.volthreshold, options.ordersize, options.latency_ms, options.binary_name, options.alias)
