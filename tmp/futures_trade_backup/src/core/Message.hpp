...
namespace simulator
{
	...
	// check if book is good
	bool isBookGood() const
	{
	    if(!bidPx_.empty() && !askPx_.empty() &&
	       bidPx_[0].getPx() > 0 && askPx_[0].getPx() > 0 &&
	       bidQty_.front() > 0 && askQty_.front() > 0 &&
	       bidPx_[0].getPx() <= askPx_[0].getPx() )
		return true;
	    else
		return false;
