...
namespace simulator
{
    ...
    Strategy::Strategy(const string strat_name) :
	strategyName_(strat_name)
    {}

    void Strategy::setSimulatorCore(SimulatorCore * simcore_ptr)
    {
	simulatorCorePtr_ = simcore_ptr;
    }

    void Strategy::setLogger(Logger * logger_ptr)
    {
	loggerPtr_ = logger_ptr;
    }
	...
	if (loggerPtr_) {
	    if (!headerLogged_) {
		loggerPtr_->push(getStrategyStatusHeader(header));
		headerLogged_ = true;
	    }
	    loggerPtr_->push(getStrategyStatus(content));
	}
    }

    string Strategy::getStrategyStatusHeader(std::string header)
    {
	stringstream ss;
	ss << "Name" << ","
	   << "Time" << ","
    ...
    string Strategy::getStrategyStatus(string content)
    {
	stringstream ss;
	ss << "StrategyStatus" << ","
