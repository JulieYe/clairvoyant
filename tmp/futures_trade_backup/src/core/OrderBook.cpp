...
namespace simulator
{
  ...
  OrderBook::OrderBook()
    ...
    void OrderBook::stashCancelOrder(CancelOrderMessage<MarketOrderMessage> const * ptr)
    {
	stashedCancelMarketOrderMessages_.push_back(*ptr);
	sort(stashedCancelMarketOrderMessages_.begin(),
	     stashedCancelMarketOrderMessages_.end(),
	     [](CancelOrderMessage<MarketOrderMessage> l, CancelOrderMessage<MarketOrderMessage> r) { return l.getTimestamp() < r.getTimestamp(); });
    }

    void OrderBook::stashCancelOrder(CancelOrderMessage<LimitOrderMessage> const * ptr)
    {
	stashedCancelLimitOrderMessages_.push_back(*ptr);
	sort(stashedCancelLimitOrderMessages_.begin(),
	     stashedCancelLimitOrderMessages_.end(),
	     [](CancelOrderMessage<LimitOrderMessage> l, CancelOrderMessage<LimitOrderMessage> r) { return l.getTimestamp() < r.getTimestamp(); });
    }

    void OrderBook::processStashedCancelOrderMessages(time_stamp newtime, std::list<CanceledOrderMessage<MarketOrderMessage> > &cancels, 
						      std::list<RejectedOrderMessage<CancelOrderMessage<MarketOrderMessage> > > &rejects,
						      atomic_long &messageId)
    {
	for(auto & m : stashedCancelMarketOrderMessages_) {
	    if (m.getTimestamp() < newtime) {
		if(cancelOrderHelper(&m))
		    cancels.push_back(CanceledOrderMessage<MarketOrderMessage>(m.getExchangeId(), m.getContractId(), newtime, m.getStratId(), messageId++, m.getOrderId(), m.getQty(), m.getSide()));
		else 
		    rejects.push_back(RejectedOrderMessage<CancelOrderMessage<MarketOrderMessage> >(m.getExchangeId(), m.getContractId(), newtime, m.getStratId(), messageId++, m.getOrderId(), m.getQty(), m.getSide()));
	    }
	}
	stashedCancelMarketOrderMessages_.erase(remove_if(stashedCancelMarketOrderMessages_.begin(),
							  stashedCancelMarketOrderMessages_.end(),
							  [&](CancelOrderMessage<MarketOrderMessage> m) {return m.getTimestamp() < newtime; }),
						stashedCancelMarketOrderMessages_.end());
    }
    
    void OrderBook::processStashedCancelOrderMessages(time_stamp newtime, std::list<CanceledOrderMessage<LimitOrderMessage> > &cancels, 
						      std::list<RejectedOrderMessage<CancelOrderMessage<LimitOrderMessage> > > &rejects,
						      atomic_long &messageId)
    {
	for(auto & m : stashedCancelLimitOrderMessages_) {
	    if (m.getTimestamp() < newtime) {
		if(cancelOrderHelper(&m))
		    cancels.push_back(CanceledOrderMessage<LimitOrderMessage>(m.getExchangeId(), m.getContractId(), newtime, m.getStratId(), messageId++, m.getOrderId(), m.getQty(), m.getSide(), m.getPx()));
		else 
		    rejects.push_back(RejectedOrderMessage<CancelOrderMessage<LimitOrderMessage> >(m.getExchangeId(), m.getContractId(), newtime, m.getStratId(), messageId++, m.getOrderId(), m.getQty(), m.getSide(), m.getPx()));
	    }
	}
	stashedCancelLimitOrderMessages_.erase(remove_if(stashedCancelLimitOrderMessages_.begin(),
							  stashedCancelLimitOrderMessages_.end(),
							  [&](CancelOrderMessage<LimitOrderMessage> m) {return m.getTimestamp() < newtime; }),
						stashedCancelLimitOrderMessages_.end());
    }
