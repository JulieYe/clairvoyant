...
namespace simulator
{
	...
	Strategy(const string strat_name);
	void setSimulatorCore(SimulatorCore * simcore_ptr);
	void setLogger(Logger * logger_ptr);
	...
	string getStrategyStatus(std::string content);
	string getStrategyStatusHeader(std::string header);
