...
namespace simulator
{
    ...
    Matchy::Matchy(string exchangeId) :
	exchangeId_(exchangeId), messageId_(0),
	  isMarketDataReady_(false), isTraderReady_(false), actionFlag_(2)
    {}

    void Matchy::setContract(string contract)
    { 
	contractId_ = contract;
    }

    void Matchy::setSimulatorCore(SimulatorCore * simcore_ptr)
    {
	simulatorCorePtr_ = simcore_ptr;
    }

    void Matchy::getParamFromConfig(INIReader & iniconfig, string sectionName)
    {}
	...
	time_stamp tp = simulatorCorePtr_->convertStringToTimePoint(datetime) + mdOffset_;
    ...
    SimMatchy::SimMatchy(string exchange) :
	Matchy(exchange)
    { }
    ...
    void SimMatchy::setLatency(long ms)
    {
	latency_ = duration_nano(ms * 1000000ULL);
    }
    
    void SimMatchy::setMdOffset(long ms)
    {
	mdOffset_ = duration_nano(ms * 1000000ULL);
    }
    ...
    void SimMatchy::stashIncomingMessages()
    {
	for (auto & m : marketOrderMessages_) {
	    m.addLatency(latency_);
    ...
    void SimMatchy::processMarketOrder(time_stamp worldtime, time_stamp localtime)
    {
	...
	partition_copy(marketOrderBook.begin(),
		       ...
		       [&](MarketOrderMessage m) { return m.getTimestamp() <= worldtime && m.getTimestamp() < localtime; });
	...
	matchMarketOrder(currBookUpdate_, mob_in, worldtime);
    ...
    void SimMatchy::processMarketableLimitOrder(time_stamp worldtime, time_stamp localtime, time_stamp recentbooktime)
    {
	{
		...
		partition_copy(bidLimitOrder.begin(),
			       ...
			       [&](LimitOrderMessage m) { return m.getTimestamp() >= recentbooktime && m.getTimestamp() < localtime; });
	    }
	    ...
	    /* Fill order and append to filledOrderMessages_. */
	    matchMarketableLimitOrder(currBookUpdate_, lob_in, lob_out, worldtime, BuySellSide::buy);
	    ...
	    sort(lob_in.begin(),
		 ...
		 [] (LimitOrderMessage l, LimitOrderMessage r) { return l.getTimestamp() < r.getTimestamp(); });
	    /* Fill order and append to filledOrderMessages_. */
	    matchMarketableLimitOrder(currBookUpdate_, lob_in, lob_out, worldtime, BuySellSide::sell);
    ...
    void SimMatchy::processLimitOrderInOrderBook(time_stamp worldtime, time_stamp localtime, time_stamp recentbooktime)
    {
	...
	for (auto & bidLimitOrder : bidLimitOrderBook) {
	    partition_copy(bidLimitOrder.begin(),
			   ...
			   back_inserter(lob_in),
			   back_inserter(lob_out),
			   [&](LimitOrderMessage m) { return m.getTimestamp() <= worldtime && m.getTimestamp() < localtime; });
	}
	/* Fill order and append to filledOrderMessages_. */
	for (auto & m : lob_in) {
	    if (m.getTimestamp() < recentbooktime && m.getPx() >= currBookUpdate_.getAskPx().front()) {
		FilledOrderMessage fom(m.getExchangeId(), m.getContractId(), worldtime, m.getStratId(), messageId_++, 
	...
	/* Fill order and append to filledOrderMessages_. */
	for (auto & m : lob_in) {
	    if (m.getTimestamp() < recentbooktime && m.getPx() <= currBookUpdate_.getBidPx().front()) {
		FilledOrderMessage fom(m.getExchangeId(), m.getContractId(), worldtime, m.getStratId(), messageId_++, 
	...
	while (!end() && currBookUpdate_.getTimestamp() <= timestampSC) {
	    ...
	    // marketable limit order will match with most recent book; and only orders >= recentbooktime and < localtime and <= worldtime get to match
	    processMarketableLimitOrder(timestampSC, currTime_, timestampFreeze);
    ...
    void SimMatchy::matchMarketableLimitOrder(BookUpdate<true> & bookUpdate, vector<LimitOrderMessage> & lob_in, vector<LimitOrderMessage> & lob_out, time_stamp & ts, BuySellSide side)
    {
