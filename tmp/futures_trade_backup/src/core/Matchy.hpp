...
namespace simulator
{
	...
	Matchy(string exchange);
	virtual void setContract(string contract);
	void setSimulatorCore(SimulatorCore * simcore_ptr);

	virtual void getParamFromConfig(INIReader & iniconfig, string sectionName);
	...
	duration_nano latency_ = duration_nano(0);
	duration_nano mdOffset_ = duration_nano(0);
	...
	SimMatchy(string exchange);

	void setLatency(long ms);
	void setMdOffset(long ms);
    ...
    protected:
	void stashIncomingMessages();
	virtual void processMarketOrder(time_stamp worldtime, time_stamp localtime);
	virtual void processMarketableLimitOrder(time_stamp worldtime, time_stamp localtime, time_stamp recentbooktime);
	virtual void processLimitOrderInOrderBook(time_stamp worldtime, time_stamp localtime, time_stamp recentbooktime);
	
    protected:
	virtual void matchMarketOrder(BookUpdate<true> &, vector<MarketOrderMessage> &, time_stamp &);
	virtual void matchMarketableLimitOrder(BookUpdate<true> &, vector<LimitOrderMessage> &, vector<LimitOrderMessage> &, time_stamp &, BuySellSide);
    };   
