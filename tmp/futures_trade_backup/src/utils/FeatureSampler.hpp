...
namespace simulator
{
	...
	duration_nano sampleFrequency_;
	
	bool doIgnorePeriod_;
	time_stamp ignoreBeginTs_;
	time_stamp ignoreEndTs_;


    public:
	FeatureSampler();
	FeatureSampler(long sampleFrequencyMs);
	// initializer
	void init();
	...
	inline void setIgnorePeriod(time_stamp beginTs, time_stamp endTs)
	{
	    ignoreBeginTs_ = beginTs;
	    ignoreEndTs_ = endTs;
	    doIgnorePeriod_ = true;
	}
