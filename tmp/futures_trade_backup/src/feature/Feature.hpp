...
namespace simulator
{
    ...
    // spread of mid point
    class MidPointSpread : public Feature
    {
    private:
    	MidPoint frontContract_;
    	MidPoint backContract_;

    public:
    	MidPointSpread() : Feature("MidPointSpread"), frontContract_(), backContract_()
    	{}

    	void update(BookUpdate<true> book, bool isFrontContractQ)
    	{
	    if (isFrontContractQ) {
		frontContract_.update(book);
	    } else {
		backContract_.update(book);
	    }
    	    if (frontContract_.isGood() && backContract_.isGood()) {
    		isGood_ = true;
    		value_ = backContract_.getValue() - frontContract_.getValue();
    	    } else {
    		isGood_ = false;
    	    }
    	}
    ...
    class FeatureProd : public Feature
    {
    private:
	void createFeatureName()
	{
	    stringstream ss;
	    ss << "FeatureProd-" 
	       << feature1_->getName() << "-"
	...
	FeatureProd() :
	    Feature(),
	    feature1_(new FEATURE1()), feature2_(new FEATURE2())
	{
	    createFeatureName();
	}
	...
	void update(BookUpdate<true> book) 
	{ 
	    feature1_->update(book);
	    feature2_->update(book);
	    isGood_ = feature1_->isGood() && feature2_->isGood();
	    value_ = feature1_->getValue() * feature2_->getValue();
	}
	void update(TradeUpdate<true> trade) 
	{ 
	    feature1_->update(trade);
	    feature2_->update(trade);
	    isGood_ = feature1_->isGood() && feature2_->isGood();
	    value_ = feature1_->getValue() * feature2_->getValue();
	}
	void update(BookUpdate<true> book, TradeUpdate<true> trade) 
	{ 
	    feature1_->update(book, trade);
	    feature2_->update(book, trade);
	    isGood_ = feature1_->isGood() && feature2_->isGood();
	    value_ = feature1_->getValue() * feature2_->getValue();
	}
    };
    ...
    template<class FEATURE1, class FEATURE2>
    class FeatureAgainst : public Feature
    {
    private:
	void createFeatureName()
	{
	...
	{ 
	    ...
	    feature2_->update(book);
	    isGood_ = feature1_->isGood() && feature2_->isGood();
	    value_ = feature1_->getValue() * feature2_->getValue() < 0 ? (feature2_->getValue() > 0 ? 1.0 : -1.0) : 0.0;
	}
	void update(TradeUpdate<true> trade) 
	{ 
	    feature1_->update(trade);
    ...
    template<class FEATURE1, class FEATURE2>
    class FeatureAgree : public Feature
    {
    private:
	void createFeatureName()
	{
    ...
    template<class FEATURE1, class FEATURE2>
    class FeatureAgainstLag : public Feature
    {
    private:
	void createFeatureName()
	{
	    stringstream ss;
	    ss << "FeatureAgainstLag-" 
	       << feature1_->getName() << "-"
	       << feature2_->getName() ;
	    featureName_ = ss.str();
	}
	...
	{ 
	    ...
	    feature2_->update(book);
	    isGood_ = feature1_->isGood() && feature2_->isGood();
	    value_ = laggedValue1_ * feature2_->getValue() < 0 ? (feature2_->getValue() > 0 ? 1.0 : -1.0) : 0.0;
	    laggedValue1_ = feature1_->getValue();
	}
	void update(TradeUpdate<true> trade) 
	{ 
	    feature1_->update(trade);
    ...
    template<class FEATURE1, class FEATURE2>
    class FeatureAgreeLag : public Feature
    {
    private:
	void createFeatureName()
	{
